...
export function Earn({ wallet }: { wallet: WalletState }) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-amber-500/25 bg-amber-500/[0.06] px-4 py-3 text-sm leading-relaxed text-amber-300">
      ...
      <Panel title="Pool" desc="Nado's own liquidity-provider vault — live size, from Nado's public data.">
        {poolsPending ? (
          ...
          <div className="grid grid-cols-2 gap-3 p-6 text-sm">
            <div>
              <div className="text-xs uppercase tracking-wider text-slate-500">Pool assets</div>
              <div className="tnum mt-0.5 text-lg font-medium text-slate-200">{usd(pool.assets, { compact: true })}</div>
            </div>
            <div>
              <div className="text-xs uppercase tracking-wider text-slate-500">Pool liabilities</div>
              <div className="tnum mt-0.5 text-lg font-medium text-slate-400">{usd(pool.liabilities, { compact: true })}</div>
            </div>
          ...
          <div className="p-6 text-sm text-slate-500">No pool data available right now.</div>
        )}
      </Panel>
        ...
        <>
          <Panel title="Your position">
            {minePending ? (
              ...
              <div className="grid grid-cols-2 gap-3 p-6 text-sm">
                <div>
                  <div className="text-xs uppercase tracking-wider text-slate-500">Withdrawable now</div>
                  <div className="tnum mt-0.5 text-lg font-medium text-mint-400">
                    {usd(mine?.unlockedAmount ?? 0)}
                  </div>
                </div>
                <div>
                  <div className="text-xs uppercase tracking-wider text-slate-500">Still locked</div>
                  <div className="tnum mt-0.5 text-lg font-medium text-slate-300">
          ...
          <Panel title="Deposit">
            <div className="space-y-3 p-6">
              ...
              <button
                onClick={onDeposit}
                disabled={busy}
                className="w-full rounded-lg bg-gradient-to-b from-mint-300 to-mint-500 py-2.5 text-sm font-semibold text-ink-950 disabled:opacity-50"
              >
          ...
          <Panel title="Withdraw">
            <div className="space-y-3 p-6">
              ...
              <button
                ...
                className="w-full rounded-lg border border-ink-600 py-2.5 text-sm font-medium text-slate-300 hover:border-slate-500 disabled:opacity-40"
              >
                Withdraw
              </button>
            </div>
