...
export function Hero({ tape, onExplore }: { tape: TapeStats | undefined; onExplore: () => void }) {
  ...
  return (
    <div className="rise-in mb-10">
      <span className="inline-flex items-center gap-1.5 rounded-full border border-mint-500/25 bg-mint-500/[0.08] px-3 py-1 text-[11px] font-medium text-mint-400">
        <Spark /> Zero effort. Zero experience. Just copy.
      </span>

      <h1 className="font-display mt-4 max-w-2xl text-[36px] font-semibold leading-[1.15] tracking-tight text-slate-50 sm:text-[44px]">
      ...
      <p className="mt-3 max-w-lg text-[14.5px] leading-relaxed text-slate-500">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-5">
        <button
          onClick={onExplore}
          className="rounded-lg bg-gradient-to-b from-mint-300 to-mint-500 px-5 py-2.5 text-[13px] font-semibold text-ink-950 shadow-[0_8px_30px_-8px_rgba(62,224,176,0.5)] transition-transform hover:-translate-y-0.5"
        >
          See who's winning →
        </button>
        ...
        <div className="flex items-center gap-5">
          {proof.map((p, i) => (
            <div key={p.label} className={i > 0 ? 'border-l border-ink-700/60 pl-5' : ''}>
              <div className="tnum font-display text-[17px] font-semibold leading-none text-slate-100">
                {p.value}
              </div>
              <div className="mt-0.5 text-[10.5px] text-slate-600">{p.label}</div>
            </div>
          ))}
        </div>
      </div>
